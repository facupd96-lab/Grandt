...
// LA ✕ NO CRUZA DE FECHA (18/09). ESTO ESTUVO MAL UN DIA Y HAY QUE DEJARLO
// ESCRITO. El 17/09 le puse arrastre: si no habia lista para la fecha de hoy,
// se copiaba la de la fecha anterior. El motivo era real —tildabas el martes,
// ACTUALIZAR_TODO pasaba a la fecha siguiente el viernes y perdias los tildes—
// pero la cura era peor que la enfermedad, porque la ✕ NO QUIERE DECIR "este
// jugador no me gusta": quiere decir "este no juega ESTA fecha". Arrastrarla es
// esconder al que volvio.
// Lo que paso: en la fecha 10, Ascacibar y Velasco —dos titulares de Boca que
// juegan todas— quedaron invisibles porque venian tildados de cuando Boca tenia
// la Sudamericana entre semana. Y encima callado: aparecian en un renglon gris
// al pie de la tabla, no en el lugar donde uno los busca.
// Ahora cada fecha arranca limpia. El formato lleva la fecha adentro, asi que
// una lista de otra fecha se descarta aunque quedara escrita bajo esta clave —
// que es justo lo que hizo el arrastre de ayer, y se limpia solo al abrir.
// Lo que la ✕ tenia que resolver (lesionados, suspendidos) ya lo hace el
// Ayudante de campo del juego por su cuenta: la ✕ es para lo que VOS sabes.
function cargarFuera() {
  S.fuera = new Set();
  const fObj = (D && D.fechaObjetivo != null) ? D.fechaObjetivo : null;
  try {
    const r = JSON.parse(localStorage.getItem(CLAVE_FUERA()) || 'null');
    // Solo el formato con sello de fecha, y solo si el sello es de ESTA fecha.
    // Un array pelado es del formato viejo (o del arrastre del 17/09): se tira.
    if (r && Array.isArray(r.k) && r.f === fObj) {
      r.k.forEach(v => { const p = porClave(v) || TODOS[v]; if (p) S.fuera.add(p.id); });
    }
...
function guardarFuera() {
  const fObj = (D && D.fechaObjetivo != null) ? D.fechaObjetivo : null;
  try {
    localStorage.setItem(CLAVE_FUERA(), JSON.stringify(
      { f: fObj, k: [...S.fuera].map(id => claveDe(id) || id) }));
  } catch (e) { }
}
  ...
  // mismo va en un renglon gris al pie de la tabla.
  {
    ...
    if (!S.busqueda && escondidos.length && lista.length) {
      ...
      if (tild.length) trozos.push(`${tild.length} ${tild.length === 1 ? 'escondido' : 'escondidos'} por tu ✕: ` +
        ...
        ` <button class="pie-link pie-link-fuerte" onclick="event.stopPropagation();limpiarFuera()"
            title="Destildar a todos de una y devolverlos a la tabla">devolvelos a todos</button>`);
