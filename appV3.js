...
function marcadorDe(ids, capId) {
  ...
  if (cap) {
    ...
    if (aMano != null) cinta = { estado: 'ok', valor: aMano, quien: q };
    else if (!resuelto(cap)) cinta = { estado: 'falta', valor: fichaCap(cap), quien: q };
    // regla del juego: si el capitan no suma puntos, la cinta se pierde
    else if (!(v && v.p != null)) cinta = { estado: 'perdida', valor: 0, quien: q };
    else if (VIVO && VIVO.fichas && VIVO.fichas[cap.id] != null) cinta = { estado: 'ok', valor: VIVO.fichas[cap.id], quien: q };
    else cinta = { estado: 'sindato', valor: 0, quien: q };
  }
  return {
...
function bloqueVersus() {
  ...
  const lado = (m, quien, ic, clase, extra) => `
    <div class="vs-lado ${clase}">
      ...
      <div class="vs-pts">${enJuego ? n1(m.real) : n1(m.proyeccion)}</div>
...
function estadoRescate() {
  ...
  if (!H || !H.esperados) return { fecha: V.fecha, falta: 'hist' };
  if (!V.generadoCon || V.generadoCon !== D.generado) return { fecha: V.fecha, falta: 'sello' };
  return { fecha: V.fecha, listo: true, V, esperados: H.esperados, n: H.n || 0 };
}
...
function autoCapturar() {
  ...
  try {
    ...
    if (y && y.completa && y.partidos >= (VIVO.partidos || []).length) return;
    capturarFecha(true);
  } catch (e) { }
}
...
function bloqueRescate(r) {
  ...
  return `<div class="rev-aviso">La <b>fecha ${r.fecha}</b> quedó sin foto y falta <b>dataHist.js</b>,
    que es donde el motor guarda lo que esperaba de cada uno. Corré <b>RECALCULAR.bat</b> y volvé a entrar.</div>`;
}
function pintarRevision() {
  ...
  const selector = ns.length > 1
    ? `<div class="rev-sel">${ns.map(n => `<button class="chip-filtro${n === REV_FECHA ? ' on' : ''}"
        onclick="verRevision(${n})">Fecha ${n}</button>`).join('')}</div>` : '';
  ...
  cont.innerHTML = `
    ...
    ${enCurso && !f.completa ? `<div class="rev-aviso">La fecha ${f.fecha} <b>todavía se está jugando</b>:
      lo de abajo es parcial. Cuando estén los ${f.deTotal} partidos, la foto se rehace sola.</div>` : ''}
    ${bloqueRescate(resc)}
    ${bloqueRevTres(f)}
    ${bloqueRevMotor(f)}
    ${bloqueRevMio(f)}
